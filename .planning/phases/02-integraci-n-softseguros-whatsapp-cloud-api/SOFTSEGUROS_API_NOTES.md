...
3. ~~¿`/api/pagopoliza/` tiene un endpoint optimizado?~~ **RESUELTO 2026-07-04.**
   ...
   **Mapa de campos (semántica del INFORME TÉCNICO §3):**
   ...
   - `poliza_codio_objeto_asegurado` (sic, typo en la API) = **"el riesgo"** en la jerga de DPG (el objeto asegurado, no la póliza). Para `ramo_nombre="AUTOMÓVILES"` trae la placa (ej. `LMT78B`). Confirmado 2026-07-06 corriendo el curl real contra prod.
   **CAVEATS al cablear:** (a) response trae ~150 campos incl. comisiones/PII → **whitelist estricta Capa 4** antes del LLM; (b) es endpoint `list_*` → SOLO uso código scopeado por póliza, NUNCA tool de búsqueda al LLM; (c) `sede=1047` asumido = DPG (confirmar si multi-sede).
   **✅ CABLEADO 2026-07-06:** `get_cartera_status(poliza_id)` en `app/integrations/softseguros.py` reemplaza el viejo `get_pagos` (504, ya eliminado). Devuelve `CarteraStatus | None` (Pydantic `extra="ignore"` = el whitelist de Capa 4: `fecha_pago`, `fecha_realizara_pago`, `fecha_realizo_pago`, `saldo_pendiente`, `edad_cartera`, `ramo_nombre`, `riesgo`). Consumido por `app/memory/debtor_flags.py` (L4) cuando `poliza_id` está disponible — enriquece el system prompt con `dias_mora`/`fecha_compromiso`/`saldo_pendiente`/`riesgo`, fail-open independiente del historial de casos local.
